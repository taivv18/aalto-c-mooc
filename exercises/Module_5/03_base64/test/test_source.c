...
{
	const char *files[5] = { "nonexistent.txt", "oneline.txt", "simple.bin",
				 "testi.txt" };
	...
	for (unsigned int i = 0; i < numtests; i++) {
		...
		if (!fref) {
			fclose(fstu);
			fail("[M5.03.a] Could not open the reference file '%s'",
			     reffile);
		}
...
START_TEST(test_from_base64)
{
	...
	for (unsigned int i = 0; i < numtests; i++) {
		...
		if (!fref) {
			fclose(fstu);
			fail("[M5.03.b] Could not open the reference file '%s'",
			     reffile);
		}
