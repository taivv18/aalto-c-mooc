...
{
	char *files[] = { "nonexistent.txt", "empty.txt",  "oneline.txt",
			  "lines.txt",	     "rfc791.txt", "generated.txt" };
	...
	for (i = 0; i < 6; i++) {
		...
		fail_unless(
			ret == results[i],
			"[M5.02.a] Should have returned %d for file %s. You returned %d.",
			results[i], files[i], ret);
	}
	test_complete();
}
END_TEST
...
START_TEST(test_word_count)
{
	...
	for (i = 0; i < 6; i++) {
		...
		fail_unless(
			ret == results[i],
			"[M5.02.b] Should have returned %d for file %s. You returned %d.",
			results[i], files[i], ret);
	}
	test_complete();
}
...
void generate_file(void)
{
	...
	char one_wd[6];
	sprintf(one_wd, "%c%c%c ", generated.common, generated.common,
		generated.common);
