...
void create_str(int *numbers, char *str, char *check, int r)
{
	...
	strcat(line, n);
	strcat(line, " numbers: ");
	char *starts[] = { "Printing now ", "This time I printed ",
			   "Some numbers ", line };
...
START_TEST(test_myprint)
{
	for (int n = 0; n < 3; n++) {
		...
		int count = myprint(str, numbers[0], numbers[1], numbers[2],
				    numbers[3], numbers[4]);
		...
		fail_unless(
			!ret,
			"[Task 5.5] Incorrect output with given print command, should have been \"%s\", you gave \"%s\"\n",
			check, test);
		free(test);
		free(check);
		fail_unless(
			count == 5,
			"[Task 5.5] Incorrect return value with given print command, should have been %d, you gave %d\n",
			5, count);
	}
	test_complete();
}
