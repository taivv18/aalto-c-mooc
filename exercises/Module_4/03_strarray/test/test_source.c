...
START_TEST(test_add_string)
{
	...
	for (unsigned int i = 0; i < num; i++) {
		...
		if (strcmp(arr[i], ref[i])) {
			...
			char buf2[200];
			sprintf(buf2,
				"[M4.03.b] After adding strings %s, array member %d is %s, should be %s",
				buf, i, arr[i], ref[i]);
			free_strings(arr);
			fail(buf2);
		}
	...
	for (int k = 0; k < 2; k++) {
		...
		for (unsigned int i = 0; i < num; i++) {
			if (strcmp(arr[i], ref[k][i])) {
				...
				char buf2[200];
				sprintf(buf2,
					"[M4.03.c] After adding strings {%s}, array member %d is '%s', should be '%s'",
					buf, i, arr[i], ref[k][i]);
				free_strings(arr);
				fail(buf2);
			}
	...
	for (int k = 0; k < 2; k++) {
		...
		for (unsigned int i = 0; i < num; i++) {
			if (strcmp(arr[i], ref[k][i])) {
				...
				char buf2[200];
				sprintf(buf2,
					"[M4.03.d] After adding strings {%s}, array member %d is '%s', should be '%s'",
					buf, i, arr[i], ref[k][i]);
				free_strings(arr);
				fail(buf2);
			}
