...
const struct stu students[] = {
	...
	{ "234567", "Lasse Laiska" },
	{ "777777",
	  "Hubert Blaine Wolfeschlegelsteinhausenbergerdorff" }, // this is actual name, I googled!
	{ "aaaaaaaaa", "Aku Ankka" },
		...
		const char *id, const char *name, int last)
{
	if (!q->first) {
		sprintf(buf,
			"Queue->first is NULL, even though there should be members in queue");
		return 0;
	}
	if (!q->last) {
		sprintf(buf,
			"Queue->last is NULL, even though there should be members in queue");
		return 0;
	}
	struct student *s = q->first;
	if (!s) {
		sprintf(buf,
			"Queue should have at least %d members, but q->first is NULL",
			checkidx + 1);
		return 0;
	}
	unsigned int i = 0;
	while (checkidx - i) {
		if (!s->next) {
			sprintf(buf,
				"Queue should have at least %d members, but next pointer of member %d is NULL",
				checkidx + 1, i);
			return 0;
		}
		s = s->next;
	...
	if (last && q->last != s) {
		sprintf(buf,
			"The Queue->last pointer does not point to the last member");
		return 0;
	}
	if (s->name == name) {
		sprintf(buf,
			"Memory for name at queue member %d is not allocated dynamically",
			checkidx);
		return 0;
	}
	if (last && s->next) {
		sprintf(buf,
			"Queue member %d is last, but the next pointer is not NULL",
			checkidx);
		return 0;
	}
	if (s->id[6]) {
		sprintf(buf,
			"The student ID string at queue member %d does not have terminating NULL",
			checkidx);
		return 0;
	}
	if (id && strcmp(id, s->id)) {
		...
		snprintf(
			buf, buflen,
			"The student ID at queue member %d is \"%s\", but expected \"%s\"",
			checkidx, s->id, id);
		return 0;
	}
	if (name && strcmp(name, s->name)) {
		...
		snprintf(
			buf, buflen,
			"The student name at queue member %d is \"%s\", but expected \"%s\"",
...
START_TEST(test_Queue_enqueue)
{
	Queue *q = Queue_init();
	unsigned int qlen = 0;
	for (unsigned int i = 0; i < sizeof(students) / sizeof(struct stu);
	     i++) {
		...
		char buf[200];
		int adjust =
			(ret == 0); // cover for case when addition is rejected
		if (!check_queue(buf, sizeof(buf), q, qlen - adjust,
				 students[i - adjust].id,
				 students[i - adjust].name, 1)) {
			releaseall(q, 0);
			fail("[M3.07.a] %s", buf);
		}
		if (ret)
...
START_TEST(test_Queue_dequeue)
{
	...
	while (qlen) {
		...
		char buf[200];
		while (i + 1 < sizeof(students) / sizeof(struct stu) &&
		       strlen(students[i + 1].id) > 6)
			i++;
		if (i + 1 < sizeof(students) / sizeof(struct stu) &&
		    !check_queue(buf, sizeof(buf), q, 0, students[i + 1].id,
				 students[i + 1].name, qlen == 1)) {
			releaseall(q, 0);
...
START_TEST(test_Queue_drop)
{
	...
	for (unsigned int i = 0; i < 4; i++) {
		...
		char buf[200];
		if (!check_queue(buf, sizeof(buf), q, tests[i].checkidx,
				 tests[i].checkid, NULL, tests[i].checklast)) {
			releaseall(q, 1);
			fail("[M3.07.c] After dropping item \"%s\": %s",
			     tests[i].drop, buf);
		}
	}
	releaseall(q, 1);
