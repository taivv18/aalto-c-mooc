...
{
	char *titles[] = { "Bananas", "Apples", "Strawberries", "Cars",
			   "People" };
...
START_TEST(test_create_vessel)
{
	...
	if (strcmp(v.name, name)) {
		fail("[M3.04] Wrong name! Given was %s, you had %s", name,
		     v.name);
		free(c);
	}
	if (v.length != length) {
		fail("[M3.04] Wrong length! Given was %f, you had %f", length,
		     v.length);
		free(c);
	}
	if (v.depth != depth) {
		fail("[M3.04] Wrong depth! Given was %f, you had %f", depth,
		     v.depth);
		free(c);
	}

	if (strcmp(v.crg.title, c->title)) {
		fail("[M3.04] Wrong cargo title! Given was %s, you had %s",
		     c->title, v.crg.title);
		free(c);
	}
	if (v.crg.weight != c->weight) {
		fail("[M3.04] Wrong cargo weight! Given was %f, you had %f",
		     c->weight, v.crg.weight);
		free(c);
	}
	if (v.crg.quantity != c->quantity) {
		fail("[M3.04] Wrong cargo quantity! Given was %d, you had %d",
		     c->quantity, v.crg.quantity);
		free(c);
	}
...
START_TEST(test_print_vessel)
{
	...
	sprintf(mod, "%s\n%.1f\n%.1f\n%s\n%d\n%.1f\n", v.name, v.length,
		v.depth, v.crg.title, v.crg.quantity, v.crg.weight);
