...
static char *names[] = { "ELEC-A7110", "ELEC-A7110",
			 "A little bit longer course name", "Short" };
static unsigned char grades[] = { 0, 2, 3, 5 };
static struct date dates[] = { { 1, 1, 2015 },
			       { 3, 4, 2013 },
			       { 31, 12, 2014 },
			       { 31, 12, 2014 } };

int test_oodi(char *buf, struct oodi *pr, const char *student,
	      const char *course, unsigned char grade, struct date cdate)
{
	if (pr->course == course || pr->course == NULL) {
		sprintf(buf,
			"Memory for the course string not dynamically allocated");
		return 0;
	}
	...
	if (strlen(pr->student) > 6) {
		sprintf(buf,
			"Student ID is too long. Max. length is 6 characters");
		return 0;
	}
	...
	if (strcmp(pr->student, student)) {
		...
		sprintf(buf,
			"Incorrect student ID. You: \"%s\", expected: \"%s\"",
			pr->student, student);
		return 0;
	}
	if (pr->grade != grade) {
...
START_TEST(test_init_record)
{
	...
	for (unsigned int i = 0; i < num_entries; i++) {
		...
		p->course = NULL;
		int ret = init_record(p, students[i], names[i], grades[i],
				      dates[i]);
		if (strlen(students[i]) > 6) {
			...
			char buf[200];
			buf[0] = 0;
			ret = test_oodi(buf, p, students[i], names[i],
					grades[i], dates[i]);
			if (!ret) {
				if (p->course && p->course != names[i])
					free(p->course);
				free(p);
				fail("[M3.06.a] Struct not initialized properly: %s",
				     buf);
			}
		}
		if (p->course && p->course != names[i])
...
START_TEST(test_add_record)
{
	...
	for (i = 0; i < num_entries; i++) {
		...
		char buf[200];
		buf[0] = 0;
		int ret = test_oodi(buf, &array[i], nr.student, nr.course,
				    nr.grade, nr.compdate);
		if (!ret) {
			freeall(array, i + 1);
			fail("[M3.06.b] Array member %d not initialized properly: %s",
			     i, buf);
		}
	}
	freeall(array, i);
...
int test_change(char *buf, struct oodi *array, const char *student,
		const char *course, unsigned char newg, const struct date *newd,
		unsigned int aidx, int retref)
{
	int ret =
		change_grade(array, num_entries, student, course, newg, *newd);
	if (retref == 1 && ret != 1) {
		freeall(array, num_entries);
		sprintf(buf,
			"[M3.06.c] After change_grade for existing student '%s', course '%s', you returned %d, should have returned %d",
			student, course, ret, retref);
		return 0;
	} else if (retref == 0 && ret != 0) {
		freeall(array, num_entries);
		sprintf(buf,
			"[M3.06.c] After change_grade for nonexisting student '%s', course '%s', you returned %d, should have returned %d",
			student, course, ret, retref);
		return 0;
	}

	char strbuf[40];
	if (array[aidx].grade != newg ||
	    !same_date(&array[aidx].compdate, newd)) {
		sprintf(buf,
			"[M3.06.c] After change_grade for student '%s', course '%s', new grade should be %d, new date should be %s. Your grade: %d, date: %s",
			student, course, newg, printday(strbuf, newd),
			array[aidx].grade,
			printday(strbuf, &array[aidx].compdate));
		freeall(array, num_entries);
		return 0;
	}
	return 1;
