...
START_TEST(test_set)
{
	for (int i = 0; i < 3; i++) {
		...
		fail_unless(f != NULL,
			    "[M3.05.a] setFraction returned NULL.\n");
...
START_TEST(test_compare)
{
	...
	fail_unless(compFraction(fa, fb) == 1,
		    "[M3.05.b] Comparing %u/%u and %u/%u should return 1\n", a,
		    a + 1, a, a + 2);
	fail_unless(compFraction(fb, fa) == -1,
		    "[M3.05.b] Comparing %u/%u and %u/%u should return -1\n", a,
		    a + 2, a, a + 1);

	Fraction *faa = setFraction(2 * a, 2 * (a + 1));
	fail_unless(compFraction(fa, faa) == 0,
		    "[M3.05.b] Comparing %u/%u and %u/%u should return 0\n", a,
		    a + 1, 2 * a, 2 * (a + 1));
	freeFraction(fa);
	freeFraction(fb);
	freeFraction(faa);
...
START_TEST(test_add)
{
	...
	fail_unless(
		!compFraction(fs, fm),
		"[Task 3.3.c] Sum of 2/3 and 3/4 should be %s. You returned %s.\n",
		bufM, bufS);
	freeFraction(fs);
	...
	fail_unless(
		!compFraction(fs, fn),
		"[Task 3.3.c] Sum of %s and 2/3 should be %s. You returned %s.\n",
		bufM, bufN, bufS);
	freeFraction(fa);
	freeFraction(fb);
	freeFraction(fm);
