...
int readMap(const char *filename, Game *game)
{
	...
	do {
		...
		if (ret && strlen(buffer) > 1) {
			map->tile[count] =
				malloc(sizeof(Tile) * strlen(buffer));
			...
			game->opts.mapWidth =
				strlen(buffer) - 1; // newline in the end
			count++;
		}
	} while (ret);
...
{
	game->monsters = realloc(game->monsters,
				 sizeof(Creature) * (game->numMonsters + 1));
...
START_TEST(test_createMonsters)
{
	...
	for (unsigned int i = 0; i < game.opts.numMonsters; i++) {
		...
		if (m->pos.x < 0 || m->pos.y < 0 ||
			...
			sprintf(buf,
				"[M6.08.b] Monster %d location is out of bounds: (%d,%d). Map dimensions: (%d,%d)",
				i, m->pos.x, m->pos.y, game.opts.mapWidth,
				game.opts.mapHeight);
			freeAll(&game);
			fail(buf);
		}
		...
		if (game.map.tile[m->pos.y][m->pos.x] != TILE_OPEN &&
			...
			if (game.map.tile[m->pos.y][m->pos.x] == TILE_WALL)
				...
				strcpy(buf2, "unknown");
			sprintf(buf,
				"[M6.08.b] Monster %d location (%d,%d) is on %s. Should be on open space",
		...
		for (unsigned int j = 0; j < i; j++) {
			...
			if (m->pos.x == m2->pos.x && m->pos.y == m2->pos.y) {
				sprintf(buf,
					"[M6.08.b] Monster %d is on same location as monster %d: (%d,%d)",
					i, j, m->pos.x, m->pos.y);
				freeAll(&game);
				fail(buf);
			}
		...
		if (m->maxhp == 0) {
			freeAll(&game);
			fail("[M6.08.b] Monster %d has invalid max HP, should be > 0",
			     i);
		}
		if (m->hp != m->maxhp) {
			sprintf(buf,
				"[M6.08.b] Monster %d: HP should be initially same as max HP. HP: %d, max HP: %d",
				i, (int)m->hp, m->maxhp);
			freeAll(&game);
			fail(buf);
		}
		...
		if (strlen(m->name) == 0 || strlen(m->name) > 19) {
			sprintf(buf,
				"[M6.08.b] Monster %d should have name that has at most 19 characters. Name length: %zu",
				i, strlen(m->name));
			freeAll(&game);
			fail(buf);
		}
		if (!isalpha((int)m->sign)) {
			sprintf(buf,
				"[M6.08.b] Monster %d map sign should be alphabetic character. You have: '%c'",
				i, m->sign);
			freeAll(&game);
			fail(buf);
		}
...
float moveDelta(Game *game, Point me, Point crea,
		void (*move)(struct game_st *, struct creature_st *))
{
	game->position = me;
...
int moveTests(char *buf, Game *game, Point oldpos, float moveDelta)
{
	if (fabsf(moveDelta) > 1.5) {
		sprintf(buf,
			"[M6.08.c] Monster at (%d,%d) moved more than one square. New location: (%d,%d)",
			oldpos.x, oldpos.y, game->monsters[0].pos.x,
			game->monsters[0].pos.y);
		return 0;
	}
	if (game->map.tile[game->monsters[0].pos.y][game->monsters[0].pos.x] ==
	    TILE_WALL) {
		sprintf(buf,
			"[M6.08.c] Monster at (%d,%d) moved to position with wall. New location: (%d,%d)",
			oldpos.x, oldpos.y, game->monsters[0].pos.x,
			game->monsters[0].pos.y);
		return 0;
	}
	if (game->numMonsters > 1) {
		if (game->monsters[0].pos.x == game->monsters[1].pos.x &&
			...
			sprintf(buf,
				"[M6.08.c] Monster at (%d,%d) moved to same square with other monster. New location: (%d,%d)",
				oldpos.x, oldpos.y, game->monsters[0].pos.x,
				game->monsters[0].pos.y);
			return 0;
		}
	...
	if (game->monsters[0].pos.x == game->position.x &&
		...
		sprintf(buf,
			"[M6.08.c] Monster at (%d,%d) moved to same square with player. New location: (%d,%d)",
			oldpos.x, oldpos.y, game->monsters[0].pos.x,
			game->monsters[0].pos.y);
		return 0;
	}
...
START_TEST(test_moveTowards)
{
	...
	// test on open space
	Point myloc[] = { { 1, 1 }, { 1, 1 }, { 5, 5 } };
	Point crloc[] = { { 5, 5 }, { 1, 5 }, { 1, 1 } };
	float d;
	for (unsigned int i = 0; i < (sizeof(myloc) / sizeof(Point)); i++) {
		d = moveDelta(&game, myloc[i], crloc[i], moveTowards);
		if (d >= 0) {
			sprintf(buf,
				"[M6.08.c] Character at (%d,%d), monster at (%d,%d) did not move closer, even if it could. New location: (%d,%d)",
...
START_TEST(test_moveAway)
{
	...
	// test on open space
	Point myloc[] = { { 1, 1 }, { 1, 1 }, { 5, 5 } };
	Point crloc[] = { { 3, 3 }, { 1, 3 }, { 3, 3 } };
	float d;
	for (unsigned int i = 0; i < (sizeof(myloc) / sizeof(Point)); i++) {
		d = moveDelta(&game, myloc[i], crloc[i], moveAway);
		if (d <= 0) {
			sprintf(buf,
				"[M6.08.d] Character at (%d,%d), monster at (%d,%d) did not move further, even if it could. New location: (%d,%d)",
				myloc[i].x, myloc[i].y, crloc[i].x, crloc[i].y,
				game.monsters[0].pos.x, game.monsters[0].pos.y);
			freeAll(&game);
			fail(buf);
...
int compareMap(Game *orig, Game *loaded, char *buf)
{
	...
	char buf2[16];
	sprintf(buf,
		"Loaded map differs from original (lines marked with >)\n");
	for (unsigned int i = 0; i < orig->opts.mapHeight; i++) {
		...
		for (j = 0; j < orig->opts.mapWidth; j++) {
			if (loaded->map.tile[i][j] == TILE_WALL)
				buf2[j + 1] = '#';
			else if (loaded->map.tile[i][j] == TILE_OPEN ||
				 loaded->map.tile[i][j] == TILE_ROOM)
				buf2[j + 1] = '.';
			else
				buf2[j + 1] = '?';
		}
		...
		for (j = 0; j < orig->opts.mapWidth; j++) {
			if (orig->map.tile[i][j] == TILE_WALL)
				buf2[j] = '#';
			else if (orig->map.tile[i][j] == TILE_OPEN ||
				 orig->map.tile[i][j] == TILE_ROOM)
				buf2[j] = '.';
			else
				buf2[j] = '?';
		}
