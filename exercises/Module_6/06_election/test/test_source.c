...
START_TEST(test_read_votes)
{
	...
	for (int i = 0; i < 5; i++) {
		if (!strcmp(v[i].name, "Trump") && v[i].votes != 4) {
			free(v);
			fail("[M6.06.a] Trump should have 4 votes in this test, you had %d.",
			     v[i].votes);
		} else if (!strcmp(v[i].name, "Clinton") && v[i].votes != 2) {
			free(v);
			fail("[M6.06.a] Clinton should have 2 votes in this test, you had %d.",
			     v[i].votes);
		} else if (!strcmp(v[i].name, "Sanders") && v[i].votes != 2) {
			free(v);
			fail("[M6.06.a] Sanders should have 2 votes in this test, you had %d.",
			     v[i].votes);
		} else if (!strcmp(v[i].name, "Cruz") && v[i].votes != 1) {
			free(v);
			fail("[M6.06.a] Cruz should have 2 votes in this test, you had %d.",
			     v[i].votes);
		}
	}
	free(v);
...
START_TEST(test_results)
{
	...
	fail_unless(
		!ret,
		"[Task 6.06.b] Incorrect output with given file, should have been \n\"%s\"\nyou gave \n\"%s\"\n",
		check, test);
