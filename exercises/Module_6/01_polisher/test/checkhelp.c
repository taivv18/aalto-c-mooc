...
int mycompare(const char *student, const char *model, char *infostr)
{
	...
	while (*student) {
		...
		if (!(*model)) {
			sprintf(infostr,
				"your output is longer than expected: character: '%s', position: %d, line: %d\n",
				bufS, i + 1, line);
			return -1;
		}
		if (*student != *model) {
			sprintf(infostr,
				"position: %d, line: %d, your output: '%s' , expected: '%s'\n",
				i + 1, line, bufS, bufM);
			return -1;
		}
		if (*student == '\n') {
	...
	if (*model) {
		printchar(bufM, *model);
		sprintf(infostr,
			"output correct until position: %d, line: %d, but shorter than expected. Next character should be '%s'\n",
			i + 1, line, bufM);
		return -1;
	}
	return 0;
...
int mycompare_new(const char *student, const char *model, char *infostr,
		  size_t n)
{
	int i = 0, line = 1;
	char bufS[16] = { 0 };
	...
	while (*student) {
		...
		if (!(*model)) {
			...
			snprintf(
				infostr, n,
				"your output is longer than expected at position: %d, line: %d (char: '%s')\n"
				"Your string: ...\"%s\"...\n"
				"Reference string: ...\"%s\"...",
		...
		if (*student != *model) {
			...
			snprintf(
				infostr, n,
				"difference on position: %d, line: %d (char: '%s' vs. '%s')\n"
				"Your string: ...\"%s\"...\n"
				"Reference string: ...\"%s\"...",
	...
	if (*model) {
		...
		snprintf(
			infostr, n,
			"output correct until position: %d, line: %d, but shorter than expected (missing char: '%s').\n"
			"Your string: ...\"%s\"...\n"
			"Reference string: ...\"%s\"...",
