...
static const char *refstr = "// This is a test file\n"
			    "int main(void) {\n"
			    "/* comment\n"
			    "block */\n"
			    "printf(\"juu\\n\");\n"
...
START_TEST(test_read_file)
{
	...
	if (!str)
		str = read_file("test/testifile.c");
	if (!str) {
	...
	if (mycompare_new(str, refstr, buf, OUTPUTLEN)) {
		free(str);
		fail("[M6.01.a] Invalid string from read_file(\"testifile.c\"): %s",
		     buf);
	}
	free(str);
...
static const char *ref61b = "int main(void) {\n"
			    "\n"
			    "printf(\"juu\\n\");\n"
			    "while (1)\n"
			    "{\n"
...
START_TEST(test_remove_comments)
{
	...
	if (mycompare_new(res, ref61b, buf, OUTPUTLEN)) {
		free(res);
		fail("[M6.01.b] Invalid string from remove_comments(\"testifile.c\"): %s",
		     buf);
	}
...
static const char *ref61c = "// This is a test file\n"
			    "int main(void) {\n"
			    "    /* comment\n"
			    "    block */\n"
			    "    printf(\"juu\\n\");\n"
...
START_TEST(test_indent)
{
	...
	if (mycompare_new(res, ref61c, buf, OUTPUTLEN)) {
		free(res);
		fail("[M6.01.c] Invalid string from indent(\"testifile.c\"): %s",
		     buf);
	}
