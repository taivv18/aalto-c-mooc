...
START_TEST(test_number_swap)
{
	...
	for (int i = 0; i < 3; i++) {
		...
		fail_unless(
			a == b_temp && b == a_temp,
			"[Task 2.1] number_swap(a, b) called with *a = %d and *b = %d. Values after call are: a: %d (should be %d), b: %d (should be %d)",
...
START_TEST(test_array_sum)
{
	...
	for (int i = 0; i < 3; i++) {
		...
		fail_unless(
			stu == mod,
			"[Task 2.2] array_sum() called with array %s (length: %d). You returned: %d. Should have been: %d.",
			outbuf, len, stu, mod);
	}
}
END_TEST
...
START_TEST(test_array_reader)
{
	...
	fail_unless(
		ret == size,
		"[Task 2.3] Wrong return value with input \"%s\". You returned %d, expected %d\n",
		inp, ret, size);

	for (i = 0; i < size; i++) {
		fail_unless(
			ar1[i] == s_arr[i],
			"[Task 2.3] Wrong array member with input \"%s\". You had %d as %dth item, expected %d\n",
			inp, ar1[i], i + 1, s_arr[i]);
	}

	fail_unless(
		ar1[size] == 0,
		"[Task 2.3] You have written more than %d values to array, which was the length of user input\n",
		size);
}
END_TEST
