...
START_TEST(test_es_print)
{
	...
	fail_unless(
		!ret,
		"[Task 2.9.a] Incorrect output with input \"%s\": should have been \"%s\", you gave \"%s\"\n",
		test1, mod, str);
	...
	fail_unless(
		!ret,
		"[07_altstring.a] Incorrect output with input \"%s\": should have been \"%s\", you gave \"%s\"\n",
		test2, mod2, str2);
}
END_TEST
...
START_TEST(test_es_length)
{
	...
	fail_unless(
		ret == 14,
		"[07_altstring.b] Wrong length with string \"%s\". Got %d, expected 14\n",
		test1, ret);
	...
	fail_unless(
		ret == 0,
		"[07_altstring.b] Wrong length with string \"%s\". Got %d, expected 0\n",
		test2, ret);
}
END_TEST
...
START_TEST(test_es_copy)
{
	...
	fail_unless(
		ret == 15,
		"[07_altstring.c] Wrong return value with string \"%s\". Got %d, expected 15\n",
		test1, ret);
	//char infostr[100] = "";
	...
	fail_unless(
		!ret,
		"[07_altstring.c] Copied string is incorrect with input \"%s\". Should have been \"%s\", you have \"%s\"",
		test1, ref1, buf);
	...
	fail_unless(
		ret == 0,
		"[07_altstring.c] Wrong return value with string \"%s\". Got %d, expected 0\n",
		test2, ret);
	char *ref2 = "#";
...
START_TEST(test_es_token)
{
	...
	fail_unless(
		ret == NULL,
		"[07_altstring.d] With string \"%s\" and token \':\' should have returned NULL. Remember that the end of the string here is marked with \'#\'",
		orig1);
	//char infostr[100] = "";
	...
	fail_unless(
		!ret2,
		"[07_altstring.d] Processed string is incorrect with input (\"%s\", ':'). Should have been \"%s\", you have \"%s\"",
		orig1, ref1, str1);

	ret = es_token(str2, ':');
	fail_unless(
		ret == &(str2[7]),
		"[07_altstring.d] Wrong return value with string \"%s\": Got %p, expected %p\n",
		orig2, ret, &(str2[7]));
	//infostr[0] = 0;
