...
START_TEST(test_print_strarray)
{
	...
	fail_unless(
		!ret,
		"[Task 2.9.a] Incorrect output with given array, should have been \"%s\", you gave \"%s\"\n",
		test1, str);
	char **b = arr;
	while (*arr) {
		free(*arr);
...
START_TEST(test_str_to_strarray)
{
	...
	for (n = 0; n < 4; ++n) {
		fail_unless(
			*test_arr != NULL,
			"[Task 2.9.b] The pointer in the output array at index %d should not be NULL\n",
			n);
		...
		fail_unless(
			!ret,
			"[Task 2.9.b] Incorrect output with given string, %d. string in array should have been \"%s\", you gave \"%s\"\n",
			n, *arr, *test_arr);
		test_arr++;
		arr++;
	}
	fail_unless(
		*test_arr == NULL,
		"[Task 2.9.b] The last pointer in the output array at index %d should be NULL\n",
		n);
	test_arr = test_b;
	arr = b;
	while (*test_arr) {
